...
public class Scheduler implements Runnable {
	...
	/**
	 * identify which elevator should serve
	 */
	private int bestId;
	
	private int tem=1000;// use to store the the gap between request floor to desFloor to find the smallest one
	...
	public void sendDataToFloor(ControlDate c) {
		...
		if(c.getFloor()==c.getDestinationFloor()) {
			floor.receiveDataFromElevator(c);buffer.setEvent(Events.WAITING);
		}else {
	...
	/**
	 * method to chose the best elevator to sever
	 * */
	public int BestElevator(int requestFloor,Elevator elev) {
		
			ControlDate data = elev.getDate();
			if(data.getFloorButton()) { //means this elevator is going up
				if(requestFloor>data.getFloor()) {
					int gap = Math.abs((requestFloor-data.getFloor()));
					if(gap<tem) {
						tem=gap;
						bestId=elev.getId();
					}
